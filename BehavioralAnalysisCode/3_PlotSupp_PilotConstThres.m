...
% In main folder save PlotSupp_PilotConstThres.m (this is the master script)
% Create functions folder and save:
% 1. fitPsycheCurveLogit.m
% Download data into a data folder "Pilot_ConstThres" from OSF
...
addpath(genpath('INSERT PATH WHERE SCRIPT SAVED/Pilot_ConstThres')); %path to data folder
...
goodSubs=5; %number of subjects passing threshold
    ...
    for r=1:Cruns
        ...
        load(filename);
            ...
            for t=1:size(resultMat,2)
                if resultMat(t).Side==1 && resultMat(t).cohSet==0.25 
                    cl25=cl25+1;
...
for g=1:nSubs
    ...
    if g<=goodSubs
        ...
        filename=['S', int2str(g), '_MLP.mat'];
        load(filename);

        RightGhin(g,1)=resultMatRight(size(resultMatRight,2)).cohSet;
        LeftGhin(g,1)=resultMatLeft(size(resultMatLeft,2)).cohSet;
...
for f=1:nSubs
    ...
    if f<=goodSubs
        title(['Left S', int2str(f) ' 70% Const=' num2str(thresholdLeft(3)) ' MLP=' num2str(LeftGhin(f))]) % 
    else
...
for f=1:nSubs
    ...
    if f<=goodSubs
        title(['Right S', int2str(f) ' 70% Const=' num2str(thresholdRight(3)) ' MLP=' num2str(RightGhin(f))]) % 
    else
